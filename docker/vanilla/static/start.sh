...
# the foo user, and ensure file permissions, grant sudo rights, and such
...
if [ "$(id -u)" == 0 ]; then
    ...
    # Refit the foo user to the desired the user (NB_USER)
    if id foo &>/dev/null; then
        if ! usermod --home "/home/${NB_USER}" --login "${NB_USER}" foo 2>&1 | grep "no changes" >/dev/null; then
            _log "Updated the foo user:"
            _log "- username: foo       -> ${NB_USER}"
            _log "- home dir: /home/foo -> /home/${NB_USER}"
        fi
    elif ! id -u "${NB_USER}" &>/dev/null; then
        _log "ERROR: Neither the foo user or '${NB_USER}' exists. This could be the result of stopping and starting, the container with a different NB_USER environment variable."
        exit 1
    fi
    ...
    # Move or symlink the foo home directory to the desired users home
    # directory if it doesn't already exist, and update the current working
    # directory to the new location if needed.
    if [[ "${NB_USER}" != "foo" ]]; then
        if [[ ! -e "/home/${NB_USER}" ]]; then
            _log "Attempting to copy /home/foo to /home/${NB_USER}..."
            mkdir "/home/${NB_USER}"
            if cp -a /home/foo/. "/home/${NB_USER}/"; then
                _log "Success!"
            else
                _log "Failed to copy data from /home/foo to /home/${NB_USER}!"
                _log "Attempting to symlink /home/foo to /home/${NB_USER}..."
                if ln -s /home/foo "/home/${NB_USER}"; then
                    _log "Success creating symlink!"
                else
                    _log "ERROR: Failed copy data from /home/foo to /home/${NB_USER} or to create symlink!"
                    exit 1
                fi
            fi
        fi
        # Ensure the current working directory is updated to the new path
        if [[ "${PWD}/" == "/home/foo/"* ]]; then
...
# user we started as.
    ...
    FOO_UID="$(id -u foo 2>/dev/null)" # The default UID for the foo user
    FOO_GID="$(id -g foo 2>/dev/null)" # The default GID for the foo user
    ...
    if ! whoami &>/dev/null; then
        ...
        if [[ -w /etc/passwd ]]; then
            _log "Renaming old foo user to nayvoj ($(id -u foo):$(id -g foo))"
            ...
            sed --expression="s/^foo:/nayvoj:/" /etc/passwd >/tmp/passwd
            echo "${NB_USER}:x:$(id -u):$(id -g):,,,:/home/foo:/bin/bash" >>/tmp/passwd
            ...
            if [[ "${NB_USER}" != "foo" ]]; then
                _log "WARNING: user is ${NB_USER} but home is /home/foo. You must run as root to rename the home directory!"
            fi
        else
    ...
    if [[ ! -w /home/foo ]]; then
        _log "WARNING: no write access to /home/foo. Try starting the container with group 'users' (100), e.g. using \"--group-add=users\"."
    fi
